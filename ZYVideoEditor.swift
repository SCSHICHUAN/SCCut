...
class ZYVideoEditor: NSObject {
    ...
    let w = UIScreen().bounds.size.width
    let h = UIScreen().bounds.size.width * (16/9.0)
    
    var clips:[AVAsset]!
    var clipRanges:[CMTimeRange]!
    
    //默认过渡动画时间1秒 淡入
    var trasitionTime:CMTime = CMTimeMake(value: 3, timescale: 1)
    ...
    func buildComposition() {
        ...
        muVideoComposition.frameDuration = CMTimeMake(value: 1, timescale: 60)
    ...
    func buildTransitionCompositionAndRepairVideoSize(muComposition:AVMutableComposition,muVideoComposition:AVMutableVideoComposition) {
        ...
        for (index,asset) in clips.enumerated() {
            ...
            passThroughTimeRanges.append(CMTimeRangeMake(start: startTime, duration: clipRange.duration))
            if index > 0 {
            ...
            if index + 1 < clips.count{
                transitionTimeRanges.append(CMTimeRangeMake(start: startTime, duration: trasitionTime))
            }
        }
...
//                fromLayer.setCropRectangleRamp(fromStartCropRectangle: CGRect.init(origin: .zero, size: videoSize), toEndCropRectangle: CGRect.init(origin: .zero, size: CGSize.init(width: 0, height: videoSize.height)), timeRange: timeRange)
//            }else if degressFromVideo(asset: asset) == 0
//             && natureSize.width < natureSize.width{
//                fromLayer.setCropRectangleRamp(fromStartCropRectangle: CGRect.init(origin: .zero, size: videoSize), toEndCropRectangle: CGRect.init(origin: .zero, size: CGSize.init(width: 0, height: videoSize.height)), timeRange: timeRange)
//            } else{
...
//        }
        ...
        if videoRatio == .Ratio9_16{
            if degressFromVideo(asset: asset) == 90{
                ...
                let translateToCenter = CGAffineTransform.init(translationX: videoSize.width, y:  asset.preferredTransform.ty)
                ...
                passThroughLayer.setTransform(mixedTransform, at: CMTime.zero)
                
            }else if degressFromVideo(asset: asset) == 0
                        && natureSize.width > natureSize.height{
                let height = videoSize.width * natureSize.height / natureSize.width
                let translateToCenter = CGAffineTransform.init(translationX: 0, y:  0)
                
                let scal = videoSize.width/natureSize.height
                
                
                let t = translateToCenter.scaledBy(x:scal, y: scal-0.0003)
                passThroughLayer.setTransform(t, at: CMTime.zero)
